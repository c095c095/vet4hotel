<?php
// ═══════════════════════════════════════════════════════════
// CONTACT PAGE — VET4 HOTEL
// Single Branch: Prachanivej (Head Office)
// ═══════════════════════════════════════════════════════════
?>

<!-- ═══════════════════════════════════════════════════════════════════ -->
<!-- SECTION 1: HEAD OFFICE INFO                                       -->
<!-- ═══════════════════════════════════════════════════════════════════ -->
<section class="py-12 mt-16 relative z-20">
    <div class="container mx-auto px-4 lg:px-8">
        <div class="max-w-7xl mx-auto">
            <div
                class="bg-base-100 rounded-3xl xl:border border-base-200 lg:shadow-xl shadow-base-200/40 p-6 md:p-8 lg:p-10">
                <div class="flex flex-wrap items-center justify-between gap-4 mb-8">
                    <div>
                        <h2 class="text-2xl md:text-3xl font-bold text-base-content">ข้อมูลติดต่อสาขาหลัก</h2>
                        <p class="text-base-content/60 mt-2">โรงพยาบาลสัตว์ สัตวแพทย์ 4 · ประชานิเวศน์ 1</p>
                    </div>
                </div>

                <div class="grid grid-cols-1 lg:grid-cols-3 gap-6 lg:gap-8">
                    <div class="bg-base-200/50 rounded-2xl p-6 border border-base-200">
                        <div
                            class="w-12 h-12 rounded-xl bg-primary/10 text-primary flex items-center justify-center mb-4">
                            <i data-lucide="map-pin" class="size-6"></i>
                        </div>
                        <h3 class="font-bold text-base-content mb-3">ที่อยู่</h3>
                        <div class="text-sm text-base-content/70 leading-relaxed">
                            <p>เลขที่ 5-5/1 ถนนเทศบาลรังสฤษฎ์เหนือ</p>
                            <p>ประชานิเวศน์ 1 จตุจักร</p>
                            <p>กรุงเทพฯ 10900</p>
                        </div>
                        <a href="https://www.google.com/maps?ll=13.840317,100.544365&z=17&t=m&hl=th&gl=US&mapclient=embed&cid=6771606152475334214"
                            target="_blank" rel="noopener noreferrer"
                            class="btn btn-primary btn-sm rounded-full gap-2 mt-5 w-full">
                            <i data-lucide="navigation" class="size-4"></i>
                            เปิดแผนที่
                        </a>
                    </div>

                    <div class="bg-base-200/50 rounded-2xl p-6 border border-base-200">
                        <div
                            class="w-12 h-12 rounded-xl bg-primary/10 text-primary flex items-center justify-center mb-4">
                            <i data-lucide="phone-call" class="size-6"></i>
                        </div>
                        <h3 class="font-bold text-base-content mb-3">ช่องทางติดต่อ</h3>
                        <div class="space-y-3 text-sm text-base-content/70">
                            <div>
                                <p class="font-medium text-base-content">โทรศัพท์</p>
                                <a href="tel:029538085"
                                    class="hover:text-primary transition-colors">0-2953-8085-6</a><br>
                                <a href="tel:025893204" class="hover:text-primary transition-colors">0-2589-3204</a>
                            </div>
                            <div>
                                <p class="font-medium text-base-content">แฟกซ์</p>
                                <span>0-2954-3597</span>
                            </div>
                            <div>
                                <p class="font-medium text-base-content">อีเมล</p>
                                <a href="mailto:vet4hospital@gmail.com"
                                    class="hover:text-primary transition-colors break-all">vet4hospital@gmail.com</a>
                            </div>
                        </div>
                    </div>

                    <div class="bg-base-200/50 rounded-2xl p-6 border border-base-200">
                        <div
                            class="w-12 h-12 rounded-xl bg-primary/10 text-primary flex items-center justify-center mb-4">
                            <i data-lucide="message-circle" class="size-6"></i>
                        </div>
                        <h3 class="font-bold text-base-content mb-3">แชทสอบถาม</h3>
                        <p class="text-sm text-base-content/70 leading-relaxed mb-4">
                            สะดวกส่งข้อความผ่าน LINE เพื่อสอบถามบริการ
                            และติดตามข้อมูลน้อง ๆ ได้อย่างรวดเร็ว
                        </p>
                        <a href="https://line.me/R/ti/p/@vet4" target="_blank" rel="noopener noreferrer"
                            class="btn btn-outline btn-primary btn-sm rounded-full gap-2 w-full">
                            <i data-lucide="send" class="size-4"></i>
                            LINE: @vet4
                        </a>
                        <div class="divider my-4 text-xs text-base-content/30">เวลาทำการ</div>
                        <div class="flex items-center gap-2 text-sm text-base-content/70">
                            <i data-lucide="clock" class="size-4"></i>
                            <span>09:00 - 18:00 น. (ทุกวัน)</span>
                        </div>
                    </div>
                </div>
            </div>

            <div class="max-w-5xl mx-auto mt-18">
                <div class="bg-base-100 rounded-2xl xl:border border-base-200 overflow-hidden">
                    <div class="bg-primary/5 p-6 border-b border-base-200">
                        <div class="flex items-center gap-3">
                            <i data-lucide="route" class="size-6 text-primary"></i>
                            <h3 class="text-xl font-bold text-base-content">เส้นทางมาโรงพยาบาล (สาขาประชานิเวศน์)</h3>
                        </div>
                        <p class="text-sm text-base-content/60 mt-2">
                            เลือกเส้นทางที่ใกล้คุณที่สุด แล้วกดเปิดแผนที่ได้ทันที
                        </p>
                    </div>

                    <div class="py-6 xl:p-6 grid grid-cols-1 md:grid-cols-2 gap-4">
                        <article class="card bg-base-200/50 xl:border border-base-200">
                            <div class="card-body p-5">
                                <h4 class="card-title text-base flex items-center gap-2">
                                    <i data-lucide="map" class="size-5 text-primary"></i>
                                    จากเดอะมอลล์ งามวงศ์วาน
                                </h4>
                                <ol class="list-decimal list-inside text-sm text-base-content/70 space-y-1.5 pl-1">
                                    <li>ตรงมาถึงแยกพงษ์เพชร</li>
                                    <li>เลี้ยวขวา ตรงมาประมาณ 1.9 กม. (เจอสัญญาณไฟที่ 2)</li>
                                    <li>เลี้ยวซ้ายข้ามสะพาน ตรงมาประมาณ 350 เมตร</li>
                                    <li>เลี้ยวซ้ายเข้าถนนเทศบาลรังสฤษดิ์เหนือ โรงพยาบาลอยู่ทางซ้ายมือ</li>
                                </ol>
                            </div>
                        </article>

                        <article class="card bg-base-200/50 border border-base-200">
                            <div class="card-body p-5">
                                <h4 class="card-title text-base flex items-center gap-2">
                                    <i data-lucide="map" class="size-5 text-primary"></i>
                                    จากม.ธุรกิจบัณฑิตย์
                                </h4>
                                <ol class="list-decimal list-inside text-sm text-base-content/70 space-y-1.5 pl-1">
                                    <li>ตรงมาถึงแยกพงษ์เพชร</li>
                                    <li>ตรงข้ามแยกมาประมาณ 1.9 กม. (เจอสัญญาณไฟที่ 2)</li>
                                    <li>เลี้ยวซ้ายข้ามสะพานเล็ก ตรงมาประมาณ 350 เมตร</li>
                                    <li>เลี้ยวซ้ายเข้าถนนเทศบาลรังสฤษดิ์เหนือ (อยู่ก่อนถึงตลาดประชานิเวศน์)</li>
                                </ol>
                            </div>
                        </article>

                        <article class="card bg-base-200/50 border border-base-200">
                            <div class="card-body p-5">
                                <h4 class="card-title text-base flex items-center gap-2">
                                    <i data-lucide="map" class="size-5 text-primary"></i>
                                    จากวัดเสมียนนารี
                                </h4>
                                <ol class="list-decimal list-inside text-sm text-base-content/70 space-y-1.5 pl-1">
                                    <li>ตรงมาเรื่อย ๆ ผ่านตลาดบองมาเช่</li>
                                    <li>ตรงมาผ่านแยกไฟแดง</li>
                                    <li>ตรงมาเรื่อย ๆ ประมาณ 500 เมตร เจอตลาดประชานิเวศน์ 1 ขวามือ</li>
                                    <li>เลี้ยวขวาเข้าถนนเทศบาลรังสฤษดิ์เหนือ โรงพยาบาลจะอยู่ซ้ายมือติดถนน</li>
                                </ol>
                            </div>
                        </article>

                        <article class="card bg-base-200/50 border border-base-200">
                            <div class="card-body p-5">
                                <h4 class="card-title text-base flex items-center gap-2">
                                    <i data-lucide="map" class="size-5 text-primary"></i>
                                    กรณีอยู่ไกล (เช่น พระราม 2)
                                </h4>
                                <ol class="list-decimal list-inside text-sm text-base-content/70 space-y-1.5 pl-1">
                                    <li>วิ่งเข้าถนนงามวงศ์วาน มุ่งหน้าแยกพงษ์เพชร</li>
                                    <li>ตรงข้ามแยกพงษ์เพชร ต่อประมาณ 1.9 กม.</li>
                                    <li>เลี้ยวซ้ายข้ามสะพานเล็ก แล้วตรงต่อประมาณ 350 เมตร</li>
                                    <li>เลี้ยวซ้ายเข้าถนนเทศบาลรังสฤษดิ์เหนือ โรงพยาบาลอยู่ฝั่งซ้ายมือ</li>
                                </ol>
                            </div>
                        </article>
                    </div>
                </div>
            </div>
        </div>
    </div>
</section>

<!-- ═══════════════════════════════════════════════════════════════════ -->
<!-- SECTION 2: MAP & CONTACT FORM                                     -->
<!-- ═══════════════════════════════════════════════════════════════════ -->
<section class="py-16 md:py-24 bg-base-100">
    <div class="container mx-auto px-4 lg:px-8">
        <div class="bg-base-200/30 rounded-3xl overflow-hidden border border-base-200/60 max-w-7xl mx-auto shadow-sm">
            <div class="grid grid-cols-1 lg:grid-cols-2">

                <div class="p-8 md:p-12 lg:p-16">
                    <div class="mb-10">
                        <h2 class="text-3xl font-bold text-base-content mb-4">ฝากข้อความถึงเรา</h2>
                        <p class="text-base-content/60 text-sm">
                            ต้องการสอบถามบริการ, ราคาห้องพัก, หรือฝากคำติชม กรุณากรอกแบบฟอร์มด้านล่าง
                            ทีมงานจะรีบติดต่อกลับให้เร็วที่สุดค่ะ
                        </p>
                    </div>

                    <form class="space-y-6" onsubmit="event.preventDefault(); showContactAlert();">
                        <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                            <div class="form-control">
                                <label class="label">
                                    <span class="label-text font-medium text-base-content">ชื่อ - นามสกุล <span
                                            class="text-error">*</span></span>
                                </label>
                                <div class="relative">
                                    <div class="absolute inset-y-0 left-0 pl-4 flex items-center pointer-events-none">
                                        <i data-lucide="user" class="size-5 text-base-content/40"></i>
                                    </div>
                                    <input type="text" name="name" required placeholder="คุณพ่อ/คุณแม่น้อง..."
                                        class="input input-bordered w-full pl-11 focus:input-primary bg-base-100 text-base-content placeholder-base-content/30 transition-all">
                                </div>
                            </div>

                            <div class="form-control">
                                <label class="label">
                                    <span class="label-text font-medium text-base-content">เบอร์โทรศัพท์ <span
                                            class="text-error">*</span></span>
                                </label>
                                <div class="relative">
                                    <div class="absolute inset-y-0 left-0 pl-4 flex items-center pointer-events-none">
                                        <i data-lucide="phone" class="size-5 text-base-content/40"></i>
                                    </div>
                                    <input type="tel" name="phone" required placeholder="08X-XXX-XXXX"
                                        class="input input-bordered w-full pl-11 focus:input-primary bg-base-100 text-base-content placeholder-base-content/30 transition-all">
                                </div>
                            </div>
                        </div>

                        <div class="form-control">
                            <label class="label">
                                <span class="label-text font-medium text-base-content">อีเมล (ถ้ามี)</span>
                            </label>
                            <div class="relative">
                                <div class="absolute inset-y-0 left-0 pl-4 flex items-center pointer-events-none">
                                    <i data-lucide="mail" class="size-5 text-base-content/40"></i>
                                </div>
                                <input type="email" name="email" placeholder="example@email.com"
                                    class="input input-bordered w-full pl-11 focus:input-primary bg-base-100 text-base-content placeholder-base-content/30 transition-all">
                            </div>
                        </div>

                        <div class="form-control">
                            <label class="label">
                                <span class="label-text font-medium text-base-content">เรื่องที่ต้องการติดต่อ <span
                                        class="text-error">*</span></span>
                            </label>
                            <div class="relative">
                                <div class="absolute inset-y-0 left-0 pl-4 flex items-center pointer-events-none">
                                    <i data-lucide="info" class="size-5 text-base-content/40"></i>
                                </div>
                                <select name="subject" required
                                    class="select select-bordered w-full pl-11 focus:select-primary bg-base-100 text-base-content transition-all appearance-none">
                                    <option value="" disabled selected>-- เลือกหัวข้อ --</option>
                                    <option value="booking">สอบถามการจองห้องพัก</option>
                                    <option value="pricing">สอบถามราคา / โปรโมชัน</option>
                                    <option value="care">สอบถามการดูแล / บริการเสริม</option>
                                    <option value="feedback">ข้อเสนอแนะ / ติชมบริการ</option>
                                    <option value="other">อื่น ๆ</option>
                                </select>
                            </div>
                        </div>

                        <div class="form-control">
                            <label class="label">
                                <span class="label-text font-medium text-base-content">ข้อความ <span
                                        class="text-error">*</span></span>
                            </label>
                            <textarea name="message" rows="4" required placeholder="ระบุรายละเอียดที่ต้องการสอบถาม..."
                                class="textarea textarea-bordered w-full focus:textarea-primary bg-base-100 text-base-content placeholder-base-content/30 transition-all p-4"></textarea>
                        </div>

                        <div role="alert" class="alert shadow-sm">
                            <i data-lucide="info" class="size-5"></i>
                            <span class="text-sm">หากต้องการติดต่อด่วน โปรดโทร <strong>0-2953-8085-6</strong> หรือแชท
                                <strong>LINE: @vet4</strong> ค่ะ</span>
                        </div>

                        <div>
                            <button type="submit"
                                class="btn btn-primary w-full md:w-auto px-8 rounded-xl gap-2 hover:shadow-lg hover:shadow-primary/30 transition-all">
                                <i data-lucide="send" class="size-4"></i>
                                ส่งข้อความ
                            </button>
                        </div>
                    </form>
                </div>

                <div class="bg-base-300 relative min-h-[400px] lg:min-h-full" id="map-container">
                    <iframe id="main-map"
                        src="https://www.google.com/maps/embed?pb=!1m18!1m12!1m3!1d3873.977966807751!2d100.54210521540763!3d13.840361090290221!2m3!1f0!2f0!3f0!3m2!1i1024!2i768!4f13.1!3m3!1m2!1s0x30e29ceff7b8553f%3A0x5df993eb0d245646!2sVET+4+Animal+Hospital!5e0!3m2!1sth!2sth!4v1520268866770"
                        class="absolute inset-0 w-full h-full border-0" allowfullscreen="" loading="lazy"
                        referrerpolicy="no-referrer-when-downgrade">
                    </iframe>
                </div>
            </div>
        </div>
    </div>
</section>

<!-- ═══════════════════════════════════════════════════════════════════ -->
<!-- SECTION 3: CALL TO ACTION                                         -->
<!-- ═══════════════════════════════════════════════════════════════════ -->
<section class="py-16 bg-primary/5 border-t border-primary/10 text-center">
    <div class="container mx-auto px-4">
        <h2 class="text-2xl md:text-3xl font-bold text-base-content mb-4">พร้อมพาน้อง ๆ
            มาเป็นครอบครัวเดียวกับเราหรือยัง?</h2>
        <p class="text-base-content/60 mb-8 max-w-xl mx-auto">จองห้องพักออนไลน์ได้ง่าย ๆ ผ่านระบบจองของเรา
            หรือดูรายละเอียดห้องพักทั้งหมดก่อนตัดสินใจ</p>
        <div class="flex flex-wrap items-center justify-center gap-4">
            <a href="?page=booking" class="btn btn-primary rounded-xl gap-2 shadow-lg hover:shadow-primary/30">
                <i data-lucide="calendar-check" class="size-5"></i>
                จองห้องพักเลย
            </a>
            <a href="?page=rooms"
                class="btn btn-outline border-base-300 hover:border-base-content/20 bg-base-100 rounded-xl gap-2">
                <i data-lucide="layout-grid" class="size-5"></i>
                ดูห้องพักทั้งหมด
            </a>
        </div>
    </div>
</section>

<script>
    function showContactAlert() {
        alert('ขอบคุณสำหรับข้อความค่ะ ทีมงาน VET4 จะติดต่อกลับโดยเร็วที่สุด');
    }
</script>